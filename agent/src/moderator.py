...
class StageModerator:
    def __init__(
        ...
        self.publisher = (
            None  # Initialized in run() after _handle_single_message is bound
        )
        self._publisher_ready = asyncio.Event()
        self._last_human_event_id = (
            ""  # Captured from incoming batches for backbone linking
        )
    ...
    async def _replay_history(self):
                    ...
                    for poll in batches:
                        ...
                        for i in range(poll.num_rows):
                            if id_arr[i].as_py() == self.session_id:
                                ...
                                print(
                                    f"📜 [Moderator] Session {self.session_id} started at {start_ts}."
                                )
                                found = True
                                break
                        if found:
        ...
        self.scanner = await self.fluss.create_scanner(
            self.table, start_ts=start_ts if start_ts > 0 else None
        )
    ...
    async def _handle_single_message(
        self, actor_id, content, ts, event_id="", m_type="output"
    ) -> bool:
        ...
        """
        if not self.context.add_message(
            ...
            if await self.command_dispatcher.dispatch(content, self):
                ...
                if hasattr(self, "base_budget"):
                    ...
                    if self.base_budget != 0:
                        print(
                            f"🔄 [Moderator] Human input detected. Resetting budget to {self.base_budget} steps."
                        )
            ...
            if poll.num_rows > 0:
                ...
                for _, row in df.iterrows():
                    ...
                    if sid != self.session_id:
                        continue
                    if await self._handle_single_message(
                        row["actor_id"], row["content"], row["ts"]
                    ):
    ...
    async def run(self, autonomous_steps=0):
        ...
        if session_context:
            ...
            print(
                f"📋 [Moderator] Session context injected into {len(self.agents)} agents."
            )
        ...
        if self.last_replayed_offset > 0 and self.base_budget != 0:
            self.current_steps = self.base_budget
            print(
                f"🔄 [Moderator] Resuming autonomous mode from replayed history ({self.last_replayed_offset} msgs)."
            )
        else:
            self.current_steps = 0
        ...
        while True:
            ...
            if human_interrupted or (self.current_steps != 0):
                if not human_interrupted:
                    ...
                    print(
                        f"🤖 [Autonomous Turn] {self.current_steps if self.current_steps >= 0 else 'inf'} steps remaining..."
                    )
                ...
                if is_job_done:
                    print(
                        "🎉 [Moderator] Job is complete! Pausing the multi-agent loop."
                    )
                    backbone_id = await self.publish(
                        "Moderator",
                ...
                if winner:
                    ...
                    if resp and "[WAIT]" not in resp:
                        ...
                        print(
                            f"💤 [{winner}] chose to WAIT or failed to respond. Nudging..."
                        )
                        await self.publish(
                            "Moderator",
