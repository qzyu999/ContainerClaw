...
class DiffTool(Tool):
    ...
    async def execute(
        ...
        if self.session_shell:
            ...
            return await self.session_shell.execute(
                agent_id, {"command": command}, publish_fn=publish_fn
            )
...
class TestRunnerTool(Tool):
    ...
    async def execute(
        ...
        if runner not in self.SUPPORTED_RUNNERS:
            return ToolResult(
                success=False, output="", error=f"Unknown runner: {runner}"
            )
        ...
        try:
            ...
            return ToolResult(
                ...
                output=stdout.decode(errors="replace")[
                    : config.TOOLS.output_limit_chars
                ],
                error=stderr.decode(errors="replace")[
                    : config.TOOLS.output_limit_chars // 2
                ]
...
class ProjectBoard:
    ...
    async def initialize(self):
        ...
        try:
            # Replay board events
            scanner = (
                await self.board_table.new_scan().create_record_batch_log_scanner()
            )
                ...
                for poll in batches:
                    for i in range(poll.num_rows):
                        ...
                        if action == "create":
                            self.items.append(
                                {
                                    ...
                                    "description": poll.column("description")[
                                        i
                                    ].as_py(),
                                    "status": poll.column("status")[i].as_py(),
                                    "assigned_to": poll.column("assigned_to")[i].as_py()
                                    or None,
                            ...
                            reason = (
                                poll.column("reason")[i].as_py()
                                if "reason" in poll.schema.names
                                else ""
                            )
            ...
            print(
                f"📋 [ProjectBoard] Replayed {len(self.items)} board items from Fluss."
            )
        except Exception as e:
    ...
    async def create_item(
        ...
        now = time.time()
        recent_creates = sum(
            1 for item in self.items if item["created_at"] > (now - window)
        )
    ...
    async def update_status(
        self, item_id: str, status: str, actor: str = "Moderator", reason: str = ""
    ) -> dict | None:
        for item in self.items:
            if item["id"] == item_id:
                item["status"] = status
    ...
    async def add_comment(
        self, item_id: str, author: str, category: str, content: str
    ) -> dict | None:
        ...
        active_comments = [
            c for c in self.comments.get(item_id, []) if not c.get("archived", False)
        ]
    ...
    async def _summarize_oldest(self, item_id: str, count: int | None = None):
        ...
        for c in to_summarize:
            summary_lines.append(
                f"[{c['category']}] {c['author']}: {c['content'][:trunc]}"
            )
        ...
        print(
            f"📦 [ProjectBoard] Summarized {len(to_summarize)} comments for {item_id}"
        )
...
class BoardTool(Tool):
    ...
    async def execute(
        ...
        if action == "view":
            ...
            if not item_id:
                return ToolResult(
                    success=False, output="", error="'view' requires 'item_id'."
                )
            detail = self.board.get_item_detail(item_id)
            if detail:
                return ToolResult(success=True, output=detail)
            return ToolResult(
                success=False, output="", error=f"Item {item_id} not found."
            )
        ...
        if action == "delete":
            ...
            target_item = next(
                (item for item in self.board.items if item["id"] == item_id), None
            )
            if not target_item:
                return ToolResult(
                    success=False, output="", error=f"Item {item_id} not found."
            ...
            return ToolResult(
                success=False, output="", error=f"Failed to delete {item_id}."
            )
    ...
    async def execute(
        ...
        if path.exists():
            return ToolResult(
                success=False,
                output="",
                error=f"File already exists: {params.get('path')}. Use surgical_edit to modify it.",
            )
...
class SurgicalEditTool(Tool):
    ...
    async def execute(
        ...
        if not old_str:
            return ToolResult(
                success=False, output="", error="old_str cannot be empty."
            )
    ...
    def _perform_edit(self, params: dict, path: Path) -> ToolResult:
        ...
        try:
            ...
            try:
                ...
                return ToolResult(
                    success=False,
                    output="",
                    error=f"Failed to read file with utf-8 or latin-1: {e}",
                )
        except Exception as e:
            return ToolResult(
        ...
        if count == 0:
            return ToolResult(
                success=False,
                output="",
                error="old_str not found in the file. Please provide more context or check exact spelling.",
            )
        if count > 1:
            return ToolResult(
                success=False,
                output="",
                error="old_str appears multiple times. Please provide more context to make it unique.",
            )
        ...
        try:
            ...
            return ToolResult(
                success=True,
                output=f"Successfully replaced 1 occurrence in {path.relative_to(config.WORKSPACE_ROOT)}",
                artifacts=[str(path)],
            )
        except Exception as e:
            return ToolResult(success=False, output="", error=str(e))
...
class AdvancedReadTool(Tool):
    name = "advanced_read"
    description = (
        "Reads a specific window of lines from a file with prepended line numbers."
    )
        ...
        try:
            for root, dirs, files in os.walk(base_dir):
                ...
                dirs[:] = [
                    ...
                    if not d.startswith(".")
                    and d
                    not in ("venv", "__pycache__", "node_modules", "tests", "docs")
                ]
                for file in files:
                    ...
                    try:
                        ...
                        class MapVisitor(ast.NodeVisitor):
                            ...
                            def visit_AsyncFunctionDef(self, node):
                                self.lines.append(
                                    f"{self.indent}async def {node.name}(...):"
                                )
            ...
            return ToolResult(
                ...
                output="\n".join(output_lines)
                if output_lines
                else "No Python files with classes/functions found.",
            )
        except Exception as e:
    ...
    def get_schema(self) -> dict:
        return {
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "The term/regex to search for.",
                },
                "include_glob": {
                    "type": "string",
                ...
                "page": {
                    "type": "integer",
                    "description": "1-indexed page number (50 results/page).",
                },
            },
            "required": ["query"],
        }
    ...
    async def execute(
        ...
        try:
            ...
            stdout, _ = await asyncio.wait_for(
                proc.communicate(), timeout=config.TOOL_TIMEOUTS.search
            )
            if proc.returncode not in (0, 1):
                return ToolResult(
                    success=False,
                    output="",
                    error=f"Grep failed with code {proc.returncode}",
                )
...
class LinterTool(Tool):
    ...
    def get_schema(self) -> dict:
        return {
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": f"File path relative to {config.WORKSPACE_ROOT} to lint.",
                }
            },
            "required": ["path"],
        }
    ...
    async def execute(
        ...
        try:
            ...
            stdout, stderr = await asyncio.wait_for(
                proc.communicate(), timeout=config.TOOL_TIMEOUTS.linter
            )
            if proc.returncode == 0:
...
class SessionShellTool(Tool):
    ...
    def __init__(self, sandbox_manager):
        ...
        self.sessions: dict[
            str, Any
        ] = {}  # In remote mode, this might track container state or just be empty
    ...
    def get_schema(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "timeout": {
                    "type": "integer",
                    "description": f"Optional timeout in seconds (default {config.TOOL_TIMEOUTS.shell}).",
                },
            },
            "required": ["command"],
        }
    ...
    async def execute(
        ...
        return ToolResult(
            success=(exit_code == 0),
            output=output[: config.TOOLS.output_limit_chars]
            if output
...
class ExecuteInSandboxTool(Tool):
    ...
    async def execute(
        ...
        if not image or not command:
            return ToolResult(
                success=False, output="", error="Image and command required."
            )
