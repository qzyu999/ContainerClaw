...
class SandboxManager:
    ...
    def __init__(
        ...
        self.default_target = (
            default_target or config.CONFIG.sidecar_config.default_target_id
        )
    ...
    def client(self):
        ...
        if self._client == "ERROR":
            raise RuntimeError(
                "Docker daemon is not accessible. Check socket mounts and permissions."
            )
    ...
    async def execute_local(
        self, command: str, publish_fn: Callable[[bytes], Awaitable[None]]
    ) -> tuple[int, str]:
    ...
    async def execute_ephemeral(
        self, image: str, command: str, publish_fn: Callable[[bytes], Awaitable[None]]
    ) -> tuple[int, str]:
