...
### 3.5 Durability Guarantees: What Survives a Restart

> [!IMPORTANT]
> The `_api_turns` buffer is intentionally ephemeral. **Fluss remains the single source of truth** for all durable conversation state. This section explains exactly what is and isn't durable, and why the ephemeral buffer does not create a data loss risk.

```mermaid
graph TB
    subgraph "Durable (Fluss SST)"
        F1["Tool invocation records\n'$ file_write {path: ...}'"]
        F2["Tool result records\n'[Tool Result] file_write: SUCCESS\nWrote 42 bytes...'"]
        F3["Agent final text response\n'I created hello.py and verified it works.'"]
        F4["Board mutations\n(containerclaw.board_events)"]
    end

    subgraph "Ephemeral (in-memory only)"
        E1["_api_turns[]\n(structured functionCall/functionResponse parts)"]
        E2["Gemini thought_signature fields"]
        E3["Function call id mappings"]
    end

    subgraph "Reconstructable on Restart"
        R1["all_messages rebuilt from\nFluss via _replay_history()"]
        R2["Cross-agent context fully\nrestored from Fluss log"]
    end
    ...
    style F1 fill:#14532d,stroke:#4ade80,color:#fff
    style F2 fill:#14532d,stroke:#4ade80,color:#fff
    style F3 fill:#14532d,stroke:#4ade80,color:#fff
    style F4 fill:#14532d,stroke:#4ade80,color:#fff
    style E1 fill:#78350f,stroke:#f59e0b,color:#fff
    style E2 fill:#78350f,stroke:#f59e0b,color:#fff
    style E3 fill:#78350f,stroke:#f59e0b,color:#fff
    style R1 fill:#1e3a5f,stroke:#60a5fa,color:#fff
    style R2 fill:#1e3a5f,stroke:#60a5fa,color:#fff
```

**What happens on agent restart or UI refresh:**

| Event | Effect on `_api_turns` | Effect on `all_messages` | Data loss? |
|---|---|---|---|
| Agent container restart | Buffer cleared | Rebuilt from Fluss via `_replay_history()` | ❌ No — all tool results are in Fluss |
| UI page refresh | N/A (buffer is agent-side) | `GetHistory` reads from Fluss | ❌ No |
| Mid-tool-cycle crash | Buffer lost | Partial — tools that completed before crash have results in Fluss | ⚠️ Partial — in-flight tool cycle is lost, but this is inherent to any crash mid-operation |
| Election cycle completes | Buffer intentionally cleared | All results published to Fluss | ❌ No |

**Why this is acceptable:**

1. **The buffer only lives within a single tool execution cycle.** It is created when an agent wins an election and cleared when the cycle ends. The maximum lifetime is seconds to minutes.
2. **Every tool result is published to Fluss before the next API call.** The `await self.publish(...)` calls in `_execute_with_tools()` ensure that every tool invocation and result is durably stored before the model sees them.
3. **The buffer contains only API protocol metadata** — `thought_signature` fields, function call `id` values, and the structured format of `functionCall`/`functionResponse` parts. These are not business data; they are transport-layer concerns that are only meaningful within a single API conversation turn.
4. **Crash recovery is graceful.** If the agent crashes mid-tool-cycle, `_replay_history()` rebuilds `all_messages` from Fluss. The agent resumes with full cross-agent context. The only loss is the in-flight tool cycle, which would need to be re-triggered by the next election anyway.

> [!NOTE]
> The `_api_turns` buffer is analogous to a TCP send buffer — it holds protocol-level framing data for the duration of a single exchange, while the application-level data (tool results) is durably committed to the Fluss log. Losing the TCP buffer on connection reset doesn't lose application data; similarly, losing `_api_turns` on agent restart doesn't lose tool results.
...
- **No per-turn/per-cycle tool limits** — The old `MAX_TOOLS_PER_TURN` (5) and `MAX_TOOLS_PER_CYCLE` (20) limits in `ToolDispatcher` are removed from the execution loop. With the native function calling protocol, the model self-regulates: it returns `functionCall` parts only when it genuinely needs tool output, and switches to text when it has enough results. The `MAX_TOOL_ROUNDS` config (default: 30) remains as a safety backstop against infinite loops, and represents a reasonable upper bound — agents typically complete tasks in 3–10 tool rounds. Removing the artificial per-turn limit is important because Gemini's parallel function calling can legitimately return 5+ calls in a single response (e.g., reading multiple files to understand a codebase), and capping this artificially forces the model into suboptimal sequential patterns.
...
### 4.3 Phase 3: Gateway Enhancement
...
| **`ToolDispatcher`** | `MAX_TOOLS_PER_TURN` and `MAX_TOOLS_PER_CYCLE` limits removed from the execution loop. The dispatcher itself is unchanged; the limits were enforced in `_execute_with_tools()`. `MAX_TOOL_ROUNDS` (config) retained as safety backstop. |
...
## 9. Compositional Function Calling

Gemini supports **compositional** (sequential) function calling where the model chains multiple function calls across turns, using the output of one as input to the next. This is a critical capability for ContainerClaw agents, which routinely need multi-step tool workflows: read a file → analyze it → write a fix → run tests → report results.

### 9.1 How It Works

In compositional function calling, the model uses the result of one function call to inform the arguments of the next. Unlike parallel function calling (where multiple independent calls happen in a single turn), compositional calls are inherently **sequential** — each call depends on the previous result.
...
sequenceDiagram
    ...
    Note over Agent,TD: Compositional chain: read → analyze → fix

    Agent->>Mod: functionCall: file_read({path: "main.py"})
    Mod->>TD: execute("file_read", {path: "main.py"})
    TD-->>Mod: ToolResult(success=True, output="def main():...")
    Mod->>Agent: functionResponse(id=fc-1, result={output: "def main():..."})

    Note over Agent: Model analyzes file content,<br/>decides to write a fix

    Agent->>Mod: functionCall: file_write({path: "main.py", content: "..."})
    Mod->>TD: execute("file_write", {path: "main.py", content: "..."})
    TD-->>Mod: ToolResult(success=True, output="Wrote 128 bytes")
    Mod->>Agent: functionResponse(id=fc-2, result={output: "Wrote 128 bytes"})

    Note over Agent: Model decides to verify the fix

    Agent->>Mod: functionCall: test_runner({args: "tests/"})
    Mod->>TD: execute("test_runner", {args: "tests/"})
    TD-->>Mod: ToolResult(success=True, output="5 passed")
    Mod->>Agent: functionResponse(id=fc-3, result={output: "5 passed"})

    Note over Agent: All tools succeeded,<br/>model produces text summary

    Agent-->>Mod: text: "Fixed the import bug in main.py. All 5 tests pass."
```

### 9.2 How the Architecture Supports It

The `_api_turns` buffer is designed to accumulate across rounds within a single tool execution cycle. Each round adds two entries:

1. The model's response turn (containing `functionCall` + `thought_signature`)
2. The user's `functionResponse` turn (containing structured tool results)

By the third round in the example above, `_api_turns` contains 6 entries — the complete structured exchange. When the model makes its next decision, it sees:

```
[text history from all_messages]  ← shared cross-agent context
[model: functionCall(file_read)]   ← round 1 model turn
[user: functionResponse(file content)] ← round 1 result
[model: functionCall(file_write)]  ← round 2 model turn
[user: functionResponse(write confirmation)] ← round 2 result
[model: functionCall(test_runner)] ← round 3 model turn
[user: functionResponse(test results)] ← round 3 result
```

This gives the model full visibility into the chain of actions and their results, enabling it to make informed decisions about what to do next.

### 9.3 Durability of Compositional Chains

Every step in a compositional chain is published to Fluss as it occurs:

```mermaid
graph LR
    subgraph "Ephemeral: _api_turns buffer"
        A1["functionCall(file_read)"]
        A2["functionResponse(file content)"]
        A3["functionCall(file_write)"]
        A4["functionResponse(write ok)"]
        A5["functionCall(test_runner)"]
        A6["functionResponse(5 passed)"]
    end

    subgraph "Durable: Fluss containerclaw.chatroom"
        F1["$ file_read {path: main.py}"]
        F2["[Tool Result] file_read: SUCCESS"]
        F3["$ file_write {path: main.py}"]
        F4["[Tool Result] file_write: SUCCESS"]
        F5["$ test_runner {args: tests/}"]
        F6["[Tool Result] test_runner: SUCCESS"]
        F7["Fixed the import bug..."]
    end
    ...
    style F3 fill:#14532d,stroke:#4ade80,color:#fff
    style F4 fill:#14532d,stroke:#4ade80,color:#fff
    style F5 fill:#14532d,stroke:#4ade80,color:#fff
    style F6 fill:#14532d,stroke:#4ade80,color:#fff
    style F7 fill:#14532d,stroke:#4ade80,color:#fff
```

If the agent crashes mid-chain (e.g., after `file_write` succeeds but before `test_runner` runs):
- **Fluss has the durable record** of all completed tool calls and results
- **`_replay_history()` rebuilds** `all_messages` with the partial chain's text records
- **The in-flight chain is lost** — but the work already done (files written, board updated) persists in the workspace
- **The next election cycle** can pick up where things left off, since agents can see the partial results in the shared history

This is a reasonable trade-off: the structured `_api_turns` buffer is a **protocol-level concern** (like a TCP session), while the **application-level data** (tool invocations and results) is durably stored in Fluss.

### 9.4 Why No Tool Call Limits

Compositional function calling requires removing the old artificial tool limits (`MAX_TOOLS_PER_TURN = 5`, `MAX_TOOLS_PER_CYCLE = 20`). Here's why:

| Old Limit | Problem with Compositional FC | Resolution |
|---|---|---|
| `MAX_TOOLS_PER_TURN = 5` | Gemini can return 5+ parallel `functionCall` parts in a single response (e.g., reading multiple files). Capping at 5 silently drops calls. | **Removed.** All function calls in a response are executed. |
| `MAX_TOOLS_PER_CYCLE = 20` | A realistic task like "refactor module X" might involve 15+ sequential tools: read 5 files, write 3 fixes, run tests, read results, fix failures, re-run. | **Removed.** The model self-regulates by returning text when done. |
| `MAX_TOOL_ROUNDS = 30` | Safety backstop against infinite loops. Even complex tasks complete in <15 rounds. | **Retained.** Configurable via `MAX_TOOL_ROUNDS` env var (default: 30). |

The native function calling protocol provides a natural termination mechanism: the model returns a text response (no `functionCall` parts) when it considers the task complete. This is more accurate than an arbitrary numeric limit because the model has context about what it's trying to accomplish.
...
However, the tools could be executed **in parallel** using `asyncio.gather()` for significant speedup. Deferred because sequential execution is simpler and side-effect ordering is easier to reason about (e.g., `file_write` then `test_runner` must be sequential).
