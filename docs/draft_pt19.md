...
## 7. Configuration: Toggle & Resource Management

To handle systems with varying resource constraints, the Flink-StarRocks telemetry stack is **opt-in**. It can be toggled via the `config.yaml` file under the `infrastructure` section. 

### 7.1 `config.yaml` definition
